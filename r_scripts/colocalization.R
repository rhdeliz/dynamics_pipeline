...
CostFx <- function(FrameTable){
  tryCatch({
    # Get distance
    Radius <- max(FrameTable$RADIUS, na.rm = T)
    Radius = Radius*3
...
CostTable <- mclapply(ScreenedTables, CostFx)
...
# Pull complete tables
Tables <- lapply(ColocalizationNeeded$table, fread)
Tables <- rbindlist(Tables, fill=TRUE)
ColocalizedTable <- merge(Tables, Edges, by = "UNIVERSAL_SPOT_ID", all = TRUE)
# Fill NA
ColocalizedTable[, COLOCALIZATION_GROUP := COLOCALIZATION_GROUP[!is.na(COLOCALIZATION_GROUP)][1L], by = UNIVERSAL_TRACK_ID]
...
ggplot(
  ColocalizedTable %>% filter(!is.na(COLOCALIZATION_GROUP)),
  aes(
  ...
  # facet_grid(~COLOCALIZATION_GROUP, scales = "free_x", space="free")+
  # coord_fixed(ratio = 1) +
  scale_color_brewer(
    palette = "Set1"
  ) +
